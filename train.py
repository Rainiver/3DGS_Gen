...
def subprocess_fn(rank, c, temp_dir, desc):
    ...
    if rank == 0:
        wandb.init(project="eg3d_snarf", name=desc, config=c, settings = wandb.Settings(start_method= 'fork'))
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.GSLoss')
    ...
    # c.G_kwargs.class_name = 'training.triplane.F'
    c.G_kwargs.class_name = 'training.triplane.GSGenerator'
    c.D_kwargs.class_name = 'training.discriminator.AG3DDiscriminator'
    # c.D_kwargs.class_name = 'training.discriminator.GSDiscriminator'
    ...
    if opts.cfg == 'deepfashion' or 'human':
