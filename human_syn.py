...
from smpl_model.body_models import SMPL
...
class HumanDataset(Dataset):
    def __init__(self):
        ...
        # self.H, self.W = 512, 512 # hardcoded original size
        ...
        # with open(os.path.join(self.root_dir, self.subject, 'calibration_full.json'), 'r') as f:
        #     self.cameras = json.load(f)
        ...
        model_files = os.path.join(subject_dir, 'smpl_params.npz')
        self.model_files = model_files
        # frames = list(range(len(model_files)))
        frames = list(range(2001))
        ...
        if split == 'predict' or cfg.freeview:
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                for d_idx, f_idx in enumerate(frames):
                    ...
                    model_params = np.load(model_files)
                    betas = np.array(model_params['betas'])
                    global_orient = np.array(model_params['global_orient'][d_idx])
                    transl = np.array(model_params['transl'][d_idx])
                    body_pose = np.array(model_params['body_pose'][d_idx])
                    jaw_pose = np.array(model_params['jaw_pose'][d_idx])
                    expression = np.array(model_params['expression'][d_idx])
                    left_hand_pose = np.array(model_params['left_hand_pose'][d_idx])
                    right_hand_pose = np.array(model_params['right_hand_pose'][d_idx])
                    model_path = os.path.join(subject_dir, f'models/{f_idx}.npz')
                    model_file = np.savez(model_path, betas=betas, global_orient=global_orient, transl=transl,
                                          body_pose=body_pose, jaw_pose=jaw_pose, expression=expression,
                                          left_hand_pose=left_hand_pose, right_hand_pose=right_hand_pose)

                    # get dummy gt...
                    # img_file = glob.glob(os.path.join(cam_dir, '*.jpg'))[0]
                    img_file = os.path.join(subject_dir, '22010710', '00000000.jpg')
                    # mask_file = glob.glob(os.path.join(cam_dir, '*.png'))[0]
                    mask_file = os.path.join(subject_dir, '22010710', 'mask', 'pha', '000000.png')
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                mask_files = sorted(glob.glob(os.path.join(cam_dir, 'mask', 'pha', '*.jpg')))[frame_slice]
    ...
    def get_cano_smpl_verts(self, data_path):
        ...
        print('model_dict[betas]', model_dict['betas'])
        print()
        smpl_outputs = body_model(
            betas=torch.from_numpy(np.array(model_dict['betas'])),
            body_pose=torch.from_numpy(da_pose),
        ...
        # mesh.export(f"zju_mesh/human_{i}.obj")
        ...
        minimal_shape = verts.detach().numpy().reshape(6890, 3)
        print('minimal_shape.shape', minimal_shape.shape)  # (1, 6890, 3)
    ...
    def get_smpl_data(self):
        ...
        for idx, (frame, model_file) in enumerate(zip(self.frames, self.model_files_list)):
            ...
            smpl_data['global_orient'].append(model_dict['global_orient'].astype(np.float32))  # (2001, 3)
            # smpl_data['pose_body'].append(model_dict['pose_body'].astype(np.float32))
            smpl_data['body_pose'].append(model_dict['body_pose'].astype(np.float32))  # (2001, 63)
            # smpl_data['pose_hand'].append(model_dict['pose_hand'].astype(np.float32))
            smpl_data['left_hand_pose'].append(model_dict['left_hand_pose'].astype(np.float32))  # (2001, 45)
            smpl_data['right_hand_pose'].append(model_dict['right_hand_pose'].astype(np.float32))  # (2001, 45)
            # smpl_data['trans'].append(model_dict['trans'].astype(np.float32))
            smpl_data['expression'].append(model_dict['expression'].astype(np.float32))  # (2001, 10)
            smpl_data['jaw_pose'].append(model_dict['jaw_pose'].astype(np.float32))  # (2001, 3)
            smpl_data['transl'].append(model_dict['transl'].astype(np.float32))  # (2001, 45)
    ...
    def getitem(self, idx, data_dict=None):
        ...
        K = np.array(self.cameras[cam_name]['K'], dtype=np.float32).copy().reshape(3, 3)
        dist = np.array(self.cameras[cam_name]['distCoeff'], dtype=np.float32).ravel()
        R = np.array(self.cameras[cam_name]['R'], np.float32).reshape(3, 3)
        ...
        mask = cv2.imread(mask_file, cv2.IMREAD_GRAYSCALE)
        # image = cv2.undistort(image, K, dist, None)  # 相机没有畸变
        ...
        model_dict = np.load(self.model_files[idx])
        n_smpl_points = minimal_shape.shape[0]
        # trans = model_dict['trans'].astype(np.float32)
        transl = model_dict['transl'].astype(np.float32)
        ...
        full_pose = np.zeros((1, 165))
        global_orient = model_dict['global_orient'].flatten()
        full_pose[:, :3] = global_orient
        body_pose = model_dict['body_pose'].reshape(1, 63)
        full_pose[:, 3:66] = body_pose
        jaw_pose = model_dict['jaw_pose'].reshape(1, 3)
        full_pose[:, 66:69] = jaw_pose
        # leye_pose = model_dict['leye_pose'].reshape(1, 3)
        # full_pose[:, 69:72] = leye_pose
        # reye_pose = model_dict['reye_pose'].reshape(1, 3)
        # full_pose[:, 72:75] = reye_pose
        left_hand_pose = model_dict['left_hand_pose'].reshape(1, 45)
        full_pose[:, 75:120] = left_hand_pose
        right_hand_pose = model_dict['right_hand_pose'].reshape(1, 45)
        full_pose[:, 120:] = right_hand_pose
        ...
        return Camera(
            ...
            full_pose=torch.from_numpy(full_pose)
        )
