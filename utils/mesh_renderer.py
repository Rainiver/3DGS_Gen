...
class Renderer():
    def __init__(self, image_size=256, anti_alias=False, real_cam=False):
        ...
        K = torch.from_numpy(np.array([[2500., 0., 256],
                                       [0., 2500, 256],
                                       [0., 0., 1.]])).float().unsqueeze(0).to(self.device)
        ...
        # self.cameras = PerspectiveCameras(R=R, T=t, in_ndc=False, image_size=(256, 256), device=self.device)
        self.cameras = PerspectiveCameras(focal_length=1.8, R=R, T=t, device=self.device)
        # self.cameras = FoVPerspectiveCameras(R=R, T=t, focal_length=2500, device=self.device)
