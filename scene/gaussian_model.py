...
def setup_init_gaussians(vertices, faces, uv_faces, tex_coord,
        ...
        if vertices.ndim == 2:
            ...
            vertices = \
            get_shell_verts_from_base(vertices[None], faces, normal_scale, num_shells, shrunk_ref_mesh=shrunk_ref_mesh)[
                0]
        else:
...
class GaussianModel:
    ...
    def __init__(self, use_sh=False, sh_degree=3, delay=1000, feature_dim=32):
        # self.cfg = cfg
        ...
        #     self.register_buffer("stacked_tex",
        #                          vertex_uvs.unsqueeze(0).repeat(num_shells, 1, 1).contiguous())  # verts_uv (S, P_uv, 2)
    ...
    def get_features(self):
        ...
        # print('features_dc.device', features_dc.device)
        features_rest = self._features_rest.squeeze(-1).to(features_dc.device)
        # features_rest = self._features_rest.to(features_dc.device)
        # print('features_dc.shape', features_dc.shape)  # [50000, 1, 1]
        # print('features_rest.shape', features_rest.shape) # [50000, 31]
    ...
    def get_covariance(self, scaling_modifier=1):
        if hasattr(self, 'rotation_precomp'):
            return self.covariance_activation(self.get_scaling, scaling_modifier, self.rotation_precomp.to(self.get_scaling.device))
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale=1.):
        ...
        if self.use_sh:  # false
            ...
            # features = torch.zeros((fused_color.shape[0], self.feature_dim)).float().cuda()
        ...
        self._features_dc = nn.Parameter(features[:, :, 0:3].transpose(1, 2).contiguous().requires_grad_(True))
        # print('self._features_dc_create_pcd', self._features_dc)
        ...
        self._features_rest = nn.Parameter(features[:, :, 3:].transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def from_another(self, other):
        # self._xyz = other.get_xyz.to(self.device)
        self._xyz = other.get_xyz
        # self._rotation = other.get_rotation.to(self.device)
        self._rotation = other.get_rotation

        # self._rotation = other.get_rotation.to(self.device)
        self._scaling = other.get_scaling
        self._opacity = other.get_opacity
