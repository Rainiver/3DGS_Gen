...
class HashGridwithMLP(NonRigidDeform):
    ...
    def __init__(self, metadata):
        self.config = OmegaConf.load('/data/vde/zhongyuhe/workshop/mycode/3DGS_Gen/configs/non_rigid/hashgrid.yaml')
        cfg = self.config.model.deformer.non_rigid
        super().__init__(cfg)
        self.pose_encoder = HierarchicalPoseEncoder(**cfg.pose_encoder)
        # d_cond = self.pose_encoder.n_output_dims   # 144
        d_cond = 144

        # add latent code
        # self.latent_dim = cfg.get('latent_dim', 0)  # 0
        self.latent_dim = 0
        if self.latent_dim > 0:
            d_cond += self.latent_dim
            self.frame_dict = metadata['frame_dict']
        ...
        d_out = 3 + 3 + 4
        # self.feature_dim = cfg.get('feature_dim', 0)  # 16
        self.feature_dim = 16
        d_out += self.feature_dim  # 26

        # bounding_box原始的数据给的好像是三平面相机采样范围，这个数据需要获取
        self.aabb = metadata['aabb']
        self.hashgrid = HashGrid(cfg.hashgrid)
        self.mlp = VanillaCondMLP(self.hashgrid.n_output_dims, d_cond, d_out, cfg.mlp)
        #  self.hashgrid.n_output_dims=48, d_cond=144, d_out=26
    ...
    def forward(self, gaussians, data):
        ...
        Jtrs = data['Jtrs']
        # print('Jtrs.device', Jtrs.device)
        pose_feat = self.pose_encoder(rots.float(), Jtrs.float())   # 编码SMPL的姿态和形状参数
        ...
        if self.latent_dim > 0:
            ...
            if frame_idx not in self.frame_dict:
                ...
                latent_idx = self.frame_dict[frame_idx]
            latent_idx = torch.Tensor([latent_idx]).long().to(rots.device)
            latent_code = self.latent(latent_idx)
            latent_code = latent_code.expand(pose_feat.shape[0], -1)
            pose_feat = torch.cat([pose_feat, latent_code], dim=1)
        ...
        xyz_norm = self.aabb.normalize(xyz, sym=True)
        deformed_gaussians = gaussians.clone()
        feature = self.hashgrid(xyz_norm).to(rots.device)
        ...
        deformed_gaussians._xyz = gaussians._xyz.to(rots.device) + delta_xyz

        # scale_offset = self.cfg.model.deformer.non_rigid.get('scale_offset', 'logit')
        scale_offset = 'logit'
        if scale_offset == 'logit':
            # print('gaussians._scaling.shape', gaussians._scaling.shape)  # [ 50000, 3]
            # print('delta_scale.shape', delta_scale.shape)  # [50000, 3]
            deformed_gaussians._scaling = gaussians._scaling + delta_scale
        ...
        # rot_offset = self.cfg.model.deformer.non_rigid.get('rot_offset', 'add')
        rot_offset = 'mult'
        # print('rot_offset', rot_offset)
        if rot_offset == 'add':
            deformed_gaussians._rotation = gaussians._rotation + delta_rot
        elif rot_offset == 'mult':
            q1 = delta_rot
