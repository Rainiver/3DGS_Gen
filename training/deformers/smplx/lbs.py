...
def lbs(
    ...
    shape_offset = blend_shapes(betas, shapedirs.to(device))
    posedirs = posedirs.to(device)
    parents = parents.to(device)
    lbs_weights = lbs_weights.to(device)
    J_regressor = J_regressor.to(device)

    v_shaped = v_template.to(device) + shape_offset
    ...
    if pose2rot:
        rot_mats = batch_rodrigues(pose.view(-1, 3)).view(
            [batch_size, -1, 3, 3]).to(device)
        # print('rot_mats.shape', rot_mats.shape)

        pose_feature = (rot_mats[:, 1:, :, :] - ident).view([batch_size, -1])
        # print('pose_feature.shape', pose_feature.shape)  # [2, 99]
        # print('posedirs.shape', posedirs.shape)  # [207, 20670]
    ...
    verts = v_homo[:, :, :3, 0]
    return verts, J_transformed, A, T, shape_offset, pose_offsets
