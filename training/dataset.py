...
class Dataset(torch.utils.data.Dataset):
    ...
    def image_shape(self):
        # print('raw_shape', self._raw_shape)
...
class DeepFashionDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._path):
            ...
            # self._all_fnames = {os.path.relpath(os.path.join(self._path, 'images_padding', fname), start=self._path) for fname in os.listdir(os.path.join(self._path, 'images_padding'))}

            self._all_fnames = set()  # 是集合不是列表
            with open(self._path + 'dataset.json', 'rb') as fp:
                json_data = json.load(fp)
            for frame in json_data['labels']:
                self._all_fnames.add(frame)

            # print('_all_fnames', type(self._all_fnames))
    ...
    def __getitem__(self, idx):
        ...
        # print('getitem deepfashion_dataset!')
        ...
        if self._xflip[idx]:
            ...
            # seg = seg[:, :, ::-1]
            normal = None
        # print(f"hahahahah {image.shape} {normal.shape} {self.get_label(idx)}---------------------")
    ...
    def _load_raw_labels(self):
        ...
        camera_poses = [labels[fname][0] for fname in self._image_fnames]
        smpl_params = [labels[fname][1] for fname in self._image_fnames]
...
class HumanDataset(Dataset):
    def __init__(self,
        ...
        #     fname for fname in self._all_fnames if self._file_ext(fname) in PIL.Image.EXTENSION and 'img' in fname)
        self._image_fnames = sorted(fname for fname in self._all_fnames if self._file_ext(fname) in PIL.Image.EXTENSION)
        if len(self._image_fnames) == 0:
            raise IOError('No image files found in the specified path')
